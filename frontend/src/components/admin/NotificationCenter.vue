...
<template>
  <div class="relative z-50 inline-block">
    <!-- Botón de campana -->
    <button @click="toggle" class="relative bg-white dark:bg-gray-800 p-3 rounded-full shadow-lg hover:scale-105 transition cursor-pointer">
      <span class="text-2xl">🔔</span>
      <span v-if="store.unreadCount" class="absolute -top-1 -right-1 bg-red-600 text-white text-xs rounded-full px-1.5">
        {{ store.unreadCount }}
      </span>
    </button>
    ...
    >
      <div class="flex justify-between items-center mb-2">
        ...
        <button @click="store.markAllAsRead" class="text-xs text-blue-600 dark:text-blue-400 hover:underline">Marcar todo leído</button>
      </div>
